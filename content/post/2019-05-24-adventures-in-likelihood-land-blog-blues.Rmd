...
Which is a fairly large interval!^[The official [problem solutions]((https://benlambertdotcom.files.wordpress.com/2018/08/bayesianbook_problemsanswers_final.pdf)) actually suggest a different result, but I believe that the author forgot to take the square root of the number of samples when calculating the confidence interval, resulting in a much tighter interval (than expected). Other online sources, seem to corroborate our result.]

# Inference

Inference is all about asking **what are the odds?**

We can do this using our maximum likelihood (point) estimate. Recall, that we calculate probability mass by integrating probability density over the desired ranges $0 \leq x \leq \{1, 5, 30\}$:
...
Our model thus far is:
$$
D_i \sim \text{Exp}(\lambda)
$$

This a very common lan of representing models

Looking at our (log-)likelihood function, and 95% confidence intervals, we can see that there is some uncertainty regarding our estimate of $\lambda$. In other words, there is a range of values that $\lambda$ could take besides $\hat{\lambda}$. We can't know for sure if our model is the "true" model (in fact, we can argue that there is no such thing as a true model), but we can evaluate whether a different model is more suitable (performs better, explains the data better, etc) than our first model. By proposing a second model, we can compare the two and evaluate which is more adequate.
